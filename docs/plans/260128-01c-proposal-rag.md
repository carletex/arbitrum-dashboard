# Proposal RAG with LlamaIndex (Final)
...
- Create vector indexes on LlamaIndex vector store tables.
...
- Default to HNSW for interactive queries.
- Use IVFFLAT only if dataset size or write patterns make HNSW unsuitable.
- Run `ANALYZE` after index creation to improve planner accuracy.
...
### Filters (allowlist)
- `stage`: only `forum`, `snapshot`, `tally`
- `status`: normalized to lowercase with an allowlist derived from existing stage statuses
